...
namespace transport_catalogue {
    ...
    const BusInfo TransportCatalogue::GetBusInfo(std::string_view bus_num) const {
        BusPtr bus_ptr = FindBus(bus_num);
        if (bus_ptr) {
    ...
    size_t TransportCatalogue::BusesCount() const noexcept {
        return buses_.size();
    }

    const BusInfo TransportCatalogue::CreateBusInfo(BusPtr bus_ptr) const {
    ...
    double TransportCatalogue::GetDistanceBetweenStops(StopPtr from, StopPtr to, bool is_actual) const {
    ...
    RouteLength TransportCatalogue::ComputeCircleRouteLength(const std::vector<StopPtr>& route) const {
        double length = 0;
    ...
    RouteLength TransportCatalogue::ComputeNotCircleRouteLength(const std::vector<StopPtr>& route) const {
        double length = 0;
        double actual_length = 0;
        for (auto it = route.begin(); it != route.end() - 1 && it != route.end(); ++it) {
            auto stop_ptr = *it;
    ...
    RouteLength TransportCatalogue::ComputeRouteLength(const std::vector<StopPtr>& route, bool is_circle) const {
        RouteLength result = is_circle ? this->ComputeCircleRouteLength(route) : this->ComputeNotCircleRouteLength(route);
        
        return result;
    }

    void TransportCatalogue::SetGeoDistances(const std::vector<StopPtr>& route) {
    ...
    const distances_container& TransportCatalogue::GetDistances() const {
        return actual_distances_;
    }
