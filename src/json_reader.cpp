...
namespace transport_catalogue {
    void MakeBase(const json::Document& doc, TransportCatalogue& obj) {
        ...
        auto routing_settings = ParseRoutingSettings(doc);
        map_renderer::RenderSettings render_settings = map_renderer::input::ParseRenderSettings(doc);

        map_renderer::MapRenderer renderer(render_settings);
        TransportRouter route_manager(routing_settings, obj);
        RequestHandler request_handler(obj, renderer, route_manager);
        
        const std::string file_name = requests.at("serialization_settings"s).AsMap().at("file"s).AsString();
        serialize::Serialize s(obj, routing_settings, render_settings);
        s.SerializeCatalogue(file_name);
    }

    void ProcessRequests(const json::Document& doc, TransportCatalogue& obj) {
        const json::Dict& requests = doc.GetRoot().AsMap();
        const json::Array& stat_request = requests.at("stat_requests").AsArray();
        ...
        serialize::Serialize s(obj, routing_settings, render_settings);
        s.DeserializeCatalogue(file_name);


        map_renderer::MapRenderer renderer(s.GetRenderSettings());
        TransportRouter route_manager(s.GetRoutingSettings(), obj);
        RequestHandler request_handler(obj, renderer, route_manager);

        std::ofstream out("transport_catalogue.svg");
